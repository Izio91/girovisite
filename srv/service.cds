...
service girovisiteService @(requires: 'authenticated-user')@(path: '/girovisiteService') {
  ...
  action   massiveImport(extension: String, attachment: LargeString)                  returns array of String;
  action   checkErrorsBeforeMassiveImport(extension: String, attachment: LargeString) returns array of String;
}
